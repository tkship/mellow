...
import 'package:mellow/shared/vignettes/_shared/physics/constrained_scroll_physics.dart';

enum SwipeAction { remove, favorite }

/// List item that handles right & left swiping.
///
/// [child] is the widget displayed as the swipeable content.
/// [isFavorite] indicates whether the item is favorited (affects indicator rendering).
/// [onSwipe] is called when the row is swiped left (remove) or right (favorite).
class SwipeItem extends StatefulWidget {
  ...
  final void Function(GlobalKey key, {required SwipeAction action})? onSwipe;
            ...
            child: Transform(
              ...
              child: Stack(alignment: Alignment.center, children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                      boxShadow: [
                        if (widget.isFavorite && lToR)
                          BoxShadow(color: Colors.white70.withOpacity(swipeRatio), blurRadius: 18)
                      ],
                      borderRadius: BorderRadius.circular(50),
                      color: widget.isFavorite || !lToR ? Colors.white : Colors.transparent),
                ),
                Icon(
