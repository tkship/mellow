...
/// Animated particle effects. Note that this just manages the data,
/// ParticleFieldPainter renders the effect.
class ParticleField with ChangeNotifier {
  ...
  /// Creates the "favorite" effect: blue particles exploding out from a
  /// central point (like a firework).
  ...
  void tick(Duration duration) {
    // Calculate how much time has elapsed since the last tick,
    // and calculate a multiplier from that.
    ...
    // If we started this tick with particles, then notify the
    // ParticleFieldPainter that it needs to re-paint.
    if (l > 0) {
      notifyListeners();
    }
  }
