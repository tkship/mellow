...
class OBJLoader {
  ...
  Future<VertexMesh> parse() async {
    String p = pkg_path.join(_basePath, _objPath);
    _objSource = await _bundle.loadString(p);
    _parseOBJFile();
    p = pkg_path.join(_basePath, _mtlPath);
  ...
  Future<void> _loadMTLTextures() async {
    ...
    for (var mtl in _materials.values) {
      if (mtl.texturePath != null) {
        ...
        AssetImage(pkg_path.join(_basePath, mtl.texturePath), bundle: _bundle).resolve(ImageConfiguration()).addListener(
