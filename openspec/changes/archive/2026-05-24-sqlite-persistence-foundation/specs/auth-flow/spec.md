...
### Requirement: 用户注册
系统 SHALL 允许新用户通过用户名和密码注册账号，用户数据持久化到 SQLite 数据库。

#### Scenario: 注册成功
- **WHEN** 用户输入有效用户名（≥3 字符）和密码（≥6 字符）并提交
- **THEN** 系统调用 `POST /api/v1/auth/register`，将用户数据（用户名、密码哈希）写入 SQLite `users` 表，成功后自动登录并返回 Token
...
#### Scenario: 服务重启后数据保留
- **WHEN** 服务重启后用户尝试登录
- **THEN** 之前注册的用户数据仍然存在于数据库中，登录正常工作

### Requirement: 用户登录
系统 SHALL 允许已注册用户通过用户名和密码登录，从 SQLite 数据库验证凭据。

#### Scenario: 登录成功
- **WHEN** 用户输入正确的用户名和密码并提交
- **THEN** 系统从 SQLite `users` 表查询用户并验证密码哈希，成功后返回 Token
...
### Requirement: 自动登录检测
系统 SHALL 在启动时自动检测已存储的 JWT Token 有效性，验证时从数据库查询用户。
...
系统 SHALL 在 API 请求返回 401 时自动尝试刷新 Token，刷新时从数据库查询用户。
